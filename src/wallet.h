...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 1000 Super7Coin at the same time
};
...
public:
    ...
    int Priority() const
    {
        if(tx->vout[i].nValue == DARKSEND_FEE) return -7777;
        BOOST_FOREACH(int64_t d, darkSendDenominations)
            if(tx->vout[i].nValue == d) return 10000;
        if(tx->vout[i].nValue < 1*COIN) return 7777;
